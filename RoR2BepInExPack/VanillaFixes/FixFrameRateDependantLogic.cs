...
internal class FixFrameRateDependantLogic
{
    ...
    private static void FixedUpdateHook(ILContext il)
    {
        ...
        c.Emit(OpCodes.Ldarg_0);
        c.EmitDelegate(FixedUpdateDefault);
        c.Emit(OpCodes.Ret);
    }

    private static void FixedUpdatePCMCHook(ILContext il)
    {
        var c = new ILCursor(il);

        c.Emit(OpCodes.Ldarg_0);
        c.EmitDelegate((PlayerCharacterMasterController self) =>
        {
    ...
    private static void UpdatePCMCHook(ILContext il)
    {
        var c = new ILCursor(il);
